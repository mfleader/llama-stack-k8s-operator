...
func TestReconcile(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			waitForNamedResource(t, ctrlRuntimeClient, instance.Namespace, instance.Name+"-service", service)
			deployment := &appsv1.Deployment{}
			waitForNamedResource(t, ctrlRuntimeClient, instance.Namespace, instance.Name, deployment)
			// networkpolicy := &networkingv1.NetworkPolicy{}
			// waitForNamedResource(t, ctrlRuntimeClient, instance.Namespace, instance.Name+"-network-policy",
			// 	networkpolicy)
			...
			// deployment
			// uses correct container image (from distribution)
			// has correct replica count
			// has correct volume configuration?
			// has correct resource limits
		})
	}
}

func waitForNamedResource(t *testing.T, client client.Client, namespace, name string, resource client.Object) {
	t.Helper()
	...
	require.Eventually(t, func() bool {
		...
		"timed out waiting for %T %s/%s to be available", resource, namespace, name)
}

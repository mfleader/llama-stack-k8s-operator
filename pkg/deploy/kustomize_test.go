...
// newTestResource is now provided by suite_test.go in this same package.
// Its local definition has been removed to avoid redeclaration errors.
...
// TestRenderKustomize contains all unit tests for the RenderKustomize function.
func TestRenderKustomize(t *testing.T) {
	// test case 1: happy path with a standard kustomize layout
...
`
		...
		owner := &llamav1alpha1.LlamaStackDistribution{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-instance",
				Namespace: "test-render-ns", // ADDED: Set a test namespace
			},
			Spec: llamav1alpha1.LlamaStackDistributionSpec{},
		}

		// when we call RenderKustomize
		resMap, err := RenderKustomize(fsys, manifestBasePath, owner)
	...
	// test case 2: happy path with a fallback to a default directory
...
metadata:
  name: my-app`
		...
		owner := &llamav1alpha1.LlamaStackDistribution{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-instance",
				Namespace: "test-fallback-ns", // ADDED: Set a test namespace
			},
		}

		// when we call RenderKustomize on the root path
		resMap, err := RenderKustomize(fsys, manifestBasePath, owner)
	...
	// test case 3: error path when a resource file is missing
...
`
		...
		owner := &llamav1alpha1.LlamaStackDistribution{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-instance",
				Namespace: "test-error-ns", // ADDED: Set a test namespace
			},
		}
...
func TestApplyResources(t *testing.T) {
	t.Run("happy path", func(t *testing.T) {
		...
		resMap := resmap.New() // resMap here is a value, not a pointer
	...
	t.Run("skips owner", func(t *testing.T) {
		...
		resMap := resmap.New() // resMap here is a value
		require.NoError(t, resMap.Append(desiredDeployment))
		require.NoError(t, resMap.Append(desiredSvc))
	...
	t.Run("but does not steal", func(t *testing.T) {
		...
		// Added a dummy one for compilation.
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "not owned by this instance")
	})

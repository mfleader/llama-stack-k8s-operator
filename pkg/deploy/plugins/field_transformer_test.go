// pkg/deploy/plugins/field_transformer_test.go
package plugins
...
func TestSetTargetField(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			res := newTestResource(t, "v1", "Pod", "test-pod", "", tc.initialSpec)
...
func TestTransform(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			transformer: CreateFieldTransformer(FieldTransformerConfig{
			...
			expectedSpecs: map[string]map[string]any{
				"my-deploy": func() map[string]any {
					...
					for k, v := range expectedDeploymentBaseContent {
						s[k] = v
					}
					s["replicas"] = 5
					return s
